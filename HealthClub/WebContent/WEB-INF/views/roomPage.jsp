...
<body>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title text-center">房 间 管 理</h3>
		</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
							<label>房间名称</label> <input type="text" class="form-control"
								id="roomname">
						</div>
						<div class="form-group">
							<label>房间类型</label> 
						...
						<div class="form-group">
							<label>房间容量</label> <input type="text" class="form-control"
								id="capacity">
						</div>
...
<script type="text/javascript">
	...
	function edit(id) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$("#capacity").val(data.room.capacity);
				$.each(data.roomtypeList, function(index, roomtypeList) {
		            $("#roomtype").append(  //此处向select中循环绑定数据
		    				"<option value="+roomtypeList.number+">" + roomtypeList.name+ "</option>");
		        });
				$("#roomtype").val(data.room.roomtype.number);
			}
		});		
	}
	...
	$("#btn_save").click(function() {
		...
		//初始化用户信息
		$.ajax({
			url : 'queryRoomInfoById',
			data : {
				
			},
			success : function(data) {
				$.each(data.roomtypeList, function(index, roomtypeList) {
		            $("#saveroomtype").append(  //此处向select中循环绑定数据
		    				"<option value="+roomtypeList.number+">" + roomtypeList.name+ "</option>");
		        });
	...
	$("#saveConfirmBtn").click(function() {
		...
		$.ajax({
			...
			datatype:'json',
			data : {
				roomtype : $("#saveroomtype option:selected").val(),
				capacity : $("#savecapacity").val(),
				roomname :$("#saveroomname").val()
			},
			success : function() {
				alert("添加成功！");
				//重新加载表格
